...
goog.provide('grokdoc.AutoCompleteRowData');
goog.provide('grokdoc.IconRowRenderer');
goog.provide('grokdoc.IconifiedAutoComplete');
goog.provide('grokdoc.IndexDownloader');
goog.provide('grokdoc.SectionManager');
goog.provide('grokdoc.Selector');
goog.provide('grokdoc.TabManager');
goog.provide('grokdoc.TypedTreeControl');
goog.provide('grokdoc.TypedTreeNode');
...
grokdoc.AugmentedLegend = function(legendElt) {
   ...
   * @private
   */
  this.containerElt_ = /** @type {Element} */ (legendElt.parentNode);
 ...
 * @param {string} functionType A string representing the function type,
 *     i.e. publickey.
 ...
 *
 * @param {goog.events.Event} e An event triggered by the the user's
 *     selection of an item from the autocomplete menu.
 */
...
/**
 * @return {string|undefined} The type of this node.
 */
 ...
 *     parent to child.
 * @param {goog.ui.tree.BaseNode} startNode The node from which to start
 *     traversing the tree path.
 * @param {?function(goog.ui.tree.BaseNode)} opt_eachNodeAction A function to
 *     call for each node in the path.
 * @param {?function(goog.ui.tree.BaseNode, goog.ui.tree.BaseNode)}
...
grokdoc.TypedTreeNode.followPath = function(separator, pathStr, startNode,
  ...
  if (foundNode && opt_endNodeAction) {
    opt_endNodeAction(foundNode, parent || null);
  }
};
 ...
 * @param {goog.ui.tree.TreeControl} parent The parent tree node under which to
 *     build the new tree.
 * @param {string} nodeName The name to display in the node's label.
 * @param {Array} data Data specifying the structure
...
function load() {
  ...
  function makeTabs(data) {
    ...
    tabs.addTreeToTab('Type Index', window['_typeTreeName'], typePage,
                      data['typeIndex']);
    // The file index is added under a tab named 'File Index', with a root
    // node named 'Source', within the element filePage.
    tabs.addTreeToTab('File Index', window['_fileTreeName'], filePage,
                      data['fileIndex']);
   ...
   * @param {goog.ui.tree.BaseNode} node The node represented by the current
   *     page.
   * @param {goog.ui.tree.BaseNode} parent The parent of the current page's
   *     page.
   */
  ...
  function makeSideIndex(sideIndexElt, isTypeIndex, data) {
    ...
    fullTreeData = getRoot(fullTreeData, rootPath, separator);
    // If there's only one child at the root, make a tree of that child's
    // members.
    var keys = goog.object.getKeys(fullTreeData[2]);
    var indexData;
    if (keys.length == 1) {
      indexData = fullTreeData[2][keys[0]];
    } else {
      // If more than one key, then show everything.
      indexData = fullTreeData;
    }
